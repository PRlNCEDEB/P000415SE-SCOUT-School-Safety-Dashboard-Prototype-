...
// School Admin: sees all six nav items ("Alert Testing" replaces "Submit Alert" for School Admin)
Then('I see navigation links for Dashboard, Alert Testing, Incidents, Analytics, Notifications, and Setup', async function () {
  for (const label of ['Dashboard', 'Alert Testing', 'Incidents', 'Analytics', 'Notifications', 'Setup']) {
    const el = this.page.locator(`nav a:has-text("${label}"), nav button:has-text("${label}")`).first();
    await el.waitFor({ timeout: 8000 });
  }
});

// Legacy / kept for any other scenarios that may reference old step text
