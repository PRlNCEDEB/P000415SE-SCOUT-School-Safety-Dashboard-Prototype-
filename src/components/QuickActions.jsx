...
const standardAlertTypes = [
  { value: 'Medical', icon: '🏥', desc: 'Medical emergency or first-aid needed' },
  { value: 'Lockdown', icon: '🔒', desc: 'Campus lockdown or perimeter threat' },
  { value: 'Behavior', icon: '⚠️', desc: 'Behavioral incident requiring intervention' },
  { value: 'Maintenance', icon: '🔧', desc: 'Facility issue requiring urgent attention' },
]
...
export default function QuickActions({ onSubmitAlert }) {
  ...
  // Keyboard shortcuts
  useEffect(() => {
    if (showCategory || showStandardCategory) return
    const handler = (e) => {
  ...
  }, [showCategory, showStandardCategory])
  ...
  const buildEmergencyAlertPayload = (type) => {
    const incidentType = type.value === 'Threat' ? 'lockdown' : type.value.toLowerCase()
  ...
  const handleStandardCategorySelect = (type) => {
    const incidentType = type.value === 'Behavior' ? 'behaviour' : type.value.toLowerCase()
    const priority = incidentType === 'medical' ? 'critical' : 'high'
    onSubmitAlert?.({
      type: incidentType,
      priority,
      title: `${type.value} alert triggered`,
      ...
      {showEmergencySuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white border border-gray-200 rounded-xl p-6 max-w-sm w-full shadow-xl text-center">
            ...
            <p className="text-sm text-gray-500 mb-4">The emergency incident was submitted successfully.</p>
            <button
